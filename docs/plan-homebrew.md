...
4. ~~Cut a second dummy version bump end-to-end to confirm `brew upgrade --cask markdown-editor` picks it up correctly, before considering the pipeline done.~~ **Done** on 2026-08-23: tagged `1.1.4` from `main` (`b2e08c5`, includes PR #17's Copilot-review fixes to `release.yml`). Release workflow ran green; `MarkdownEditor-1.1.4.zip` + `.sha256` attached. Tap's `Casks/markdown-editor.rb` bumped to `1.1.4` (committed directly to `main` in `homebrew-tap`, per step 9). On the owner's Mac: reset local install state to a clean `1.1.3`, ran `brew update` (cask correctly showed `1.1.3 → 1.1.4` available), then `brew upgrade --cask markdown-editor` — succeeded, backed up the old app version, installed the new one. Verified post-upgrade: `brew list --cask --versions` reports `1.1.4`, and the installed app's own `CFBundleShortVersionString` also reads `1.1.4` (confirms the `MARKETING_VERSION` override from step 4 is flowing through correctly). Ad-hoc signature confirmed present (`codesign -dv`), `spctl` rejects it as expected (no notarization — the documented "Open Anyway" flow applies). The end-to-end pipeline (tag push → release build → tap bump → `brew upgrade`) is now fully verified.
...
1. ~~**Branch protection rule on `main`**: require a pull request before merging (blocks direct `git push` to `main`, including from the owner — merging your own PR via the button after CI passes is still how you land changes); require the existing `CI / test` status check (`.github/workflows/ci.yml`) to pass before merging; block force-pushes and branch deletion on `main`; include administrators in these restrictions (so the "always use a PR" rule in `CLAUDE.md` is enforced, not just conventional).~~ **Done** 2026-08-23 via `gh api PUT .../branches/main/protection`: `required_status_checks` (strict, context `test`), `required_pull_request_reviews` (0 required approvals — see the decision above), `enforce_admins: true`, force-push and branch deletion both blocked.
2. **Actions fork-PR gate**: Settings → Actions → General → set "Fork pull request workflows from outside collaborators" to *Require approval for all outside collaborators*. CI already only triggers on `pull_request` (not `pull_request_target`) with no repo secrets configured, so this isn't plugging a secret leak — it stops random forks from running arbitrary code on your Actions minutes without you clicking "Approve and run" first. **Not done** — confirmed 2026-08-23 this setting has no REST/GraphQL API surface (checked `.../actions/permissions` and `.../actions/permissions/workflow`, neither exposes it); it's UI-only, so the owner needs to toggle it by hand in Settings.
3. ~~**Secret scanning + push protection**: enable under Settings → Code security.~~ **Done** 2026-08-23 via `gh api PATCH` on `security_and_analysis` (`secret_scanning` + `secret_scanning_push_protection`, both `enabled`).
4. ~~**Private vulnerability reporting** (optional, low effort): enable so a real security report lands privately instead of as a public issue.~~ **Done** 2026-08-23 via `gh api PUT .../private-vulnerability-reporting`.
5. ~~Add one line to `CLAUDE.md`'s workflow section noting the "never commit directly to main" rule is now GitHub-enforced (branch protection), not just convention.~~ **Done** 2026-08-23, same PR as this update.

Everything except item 2 (UI-only setting) is done.
